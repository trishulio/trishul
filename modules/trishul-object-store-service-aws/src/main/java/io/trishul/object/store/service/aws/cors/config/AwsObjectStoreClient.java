...
public class AwsObjectStoreClient implements
  ...
  public AwsObjectStoreClient(AmazonS3 awsClient,
      IaasEntityMapper<Bucket, IaasObjectStore> mapper) {
  ...
  @Override
  public <BE extends BaseIaasObjectStore<?>> IaasObjectStore add(BE objectStore) {
  ...
  @Override
  public <UE extends UpdateIaasObjectStore<?>> IaasObjectStore put(UE objectStore) {
  ...
  private Map<String, IaasObjectStore> objectStores() {
    ...
    if (cache == null) {
      ...
      if (buckets != null) {
        cache = buckets.stream().map(mapper::fromIaasEntity)
            .collect(Collectors.toMap(IaasObjectStore::getName, Function.identity()));
      }
