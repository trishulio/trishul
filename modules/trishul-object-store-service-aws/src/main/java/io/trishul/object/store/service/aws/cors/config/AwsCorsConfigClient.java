...
public class AwsCorsConfigClient
    ...
    public IaasObjectStoreCorsConfiguration get(String bucketName) {
        ...
        IaasObjectStoreCorsConfiguration iaasObjectStoreCorsConfiguration =
                bucketCrossOriginConfiguration == null
                        ? null
    ...
    public <BE extends IaasObjectStoreCorsConfiguration> IaasObjectStoreCorsConfiguration add(
            BE entity) {
        log.debug(
                "Replacing existing CORS setting on the bucket: {} (if any) since "
                        + "only single CORS configuration can be applied.",
    ...
    public <UE extends IaasObjectStoreCorsConfiguration> IaasObjectStoreCorsConfiguration put(
        ...
        final SetBucketCrossOriginConfigurationRequest request =
                new SetBucketCrossOriginConfigurationRequest(
                        entity.getBucketName(), entity.getBucketCrossOriginConfiguration());
        try {
            ...
            if (awsClient.doesBucketExistV2(entity.getBucketName())
                ...
                String msg =
                        String.format(
                                "Bucket: '%s' exists, but failed to put the cross origin "
                                        ...
                                        + "different Tenant ID for this environment.",
                                entity.getBucketName());
                log.error(msg);
            ...
            log.error(
                    "Failed to put the cross origin configuration for bucket: {}",
                    entity.getBucketName());
            throw e;
        }
