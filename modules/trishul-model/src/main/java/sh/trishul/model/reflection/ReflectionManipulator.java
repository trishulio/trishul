package sh.trishul.model.reflection;
...
import sh.trishul.base.types.lambda.CheckedFunction;
    ...
    this.propNamesCache = CacheBuilder.newBuilder().build(new CacheLoader<Class<?>, Set<String>>() {
      ...
      public Set<String> load(@NonNull Class<?> clazz) throws Exception {
        ...
        final Set<String> propertyNames = Arrays.stream(methods)
            ...
            .map(method -> method.getName().replaceFirst("get|set", "")).map(prop -> {
  ...
  public Set<String> getPropertyNames(Class<?> clazz, Set<String> exclusions) {
    ...
    try {
      ...
      throw new RuntimeException(
          String.format("Failed to fetch properties names because: %s", e.getMessage()), e);
    }
  }
