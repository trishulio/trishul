...
public class AwsIaasUserTenantMembershipClient implements
    IaasClient<IaasUserTenantMembershipId, IaasUserTenantMembership, BaseIaasUserTenantMembership<?>, UpdateIaasUserTenantMembership<?>> {
  ...
  private GroupType getGroup(String userName, String groupName) {
    return getGroups(userName).stream()
        .filter(group -> group.getGroupName().equalsIgnoreCase(groupName)).findAny().orElse(null);
  }
  ...
  private List<GroupType> getGroups(String userName) {
    ...
    do {
      AdminListGroupsForUserRequest request = new AdminListGroupsForUserRequest()
          .withUsername(userName).withNextToken(next).withUserPoolId(userPoolId);
