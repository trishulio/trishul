...
public class AwsCognitoIdentitySdkWrapper implements AwsCognitoIdentityClient {
  ...
  public List<IdentityPoolShortDescription> getIdentityPools(int pageSize) {
    ...
    do {
      ListIdentityPoolsRequest request
          = new ListIdentityPoolsRequest().withMaxResults(pageSize).withNextToken(nextToken);
  ...
  public String getIdentityId(String identityPoolId, Map<String, String> logins) {
    GetIdRequest request = new GetIdRequest().withIdentityPoolId(identityPoolId).withLogins(logins);
  ...
  public Credentials getCredentialsForIdentity(String identityId, Map<String, String> logins) {
    GetCredentialsForIdentityRequest request
        = new GetCredentialsForIdentityRequest().withIdentityId(identityId).withLogins(logins);

    GetCredentialsForIdentityResult result
        = awsCognitoIdentityClient.getCredentialsForIdentity(request);
