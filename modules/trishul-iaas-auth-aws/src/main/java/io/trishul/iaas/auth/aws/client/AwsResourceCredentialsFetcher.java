...
public class AwsResourceCredentialsFetcher implements IaasAuthorizationFetcher {
    private static final int NUM_COGNITO_ID_POOLS =
            1; // 1 because we only have 1 Cognito ID Pool per deployment.
    ...
    public AwsResourceCredentialsFetcher(
            AwsCognitoIdentityClient identityClient,
            AwsIdentityCredentialsMapper iaasAuthorizationMapper,
            String userPoolUrl) {
    ...
    public IaasAuthorization fetch(IaasAuthorizationCredentials loginCredentials) {
        ...
        String identityPoolId =
                this.identityClient
                        .getIdentityPools(NUM_COGNITO_ID_POOLS)
                        .get(0)
                        .getIdentityPoolId();
