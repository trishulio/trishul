...
@Transactional
public class IaasObjectStoreCorsConfigService extends BaseService
        implements CrudService<
                String,
                IaasObjectStoreCorsConfiguration,
                IaasObjectStoreCorsConfiguration,
                IaasObjectStoreCorsConfiguration,
                IaasObjectStoreCorsConfigurationAccessor> {
    private static final Logger log =
            LoggerFactory.getLogger(IaasObjectStoreCorsConfigService.class);
    ...
    public IaasObjectStoreCorsConfiguration get(String id) {
        ...
        List<IaasObjectStoreCorsConfiguration> bucketCrossOriginConfigs =
                this.iaasRepo.get(Set.of(id));
        if (bucketCrossOriginConfigs.size() == 1) {
            bucketCrossOriginConfig = bucketCrossOriginConfigs.get(0);
        } else {
            log.error(
                    "Unexpectedly returned more than 1 config for objectStore: {}: results {}",
                    id,
    ...
    public List<IaasObjectStoreCorsConfiguration> getByAccessorIds(
        ...
        List<IaasObjectStoreCorsConfiguration> idProviders =
                accessors.stream()
                        .filter(Objects::nonNull)
    ...
    public List<IaasObjectStoreCorsConfiguration> add(
            List<IaasObjectStoreCorsConfiguration> additions) {
        if (additions == null) {
            return null;
        }
    ...
    public List<IaasObjectStoreCorsConfiguration> put(
            List<IaasObjectStoreCorsConfiguration> updates) {
        if (updates == null) {
            return null;
        }
