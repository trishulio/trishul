...
public class AwsIamPolicyClient
  ...
  public <BE extends BaseIaasPolicy<?>> IaasPolicy add(BE addition) {
    CreatePolicyRequest request = new CreatePolicyRequest().withPolicyName(addition.getName())
        .withDescription(addition.getDescription()).withPolicyDocument(addition.getDocument());
  ...
  public <UE extends UpdateIaasPolicy<?>> IaasPolicy update(UE policy) {
    ...
    CreatePolicyVersionRequest request = new CreatePolicyVersionRequest().withPolicyArn(policyArn)
        .withPolicyDocument(policy.getDocument()).withSetAsDefault(true);

    CreatePolicyVersionResult result = awsIamClient.createPolicyVersion(request);
    log.info("Created new policy version. ARN: {}, VersionId: {}, IsDefault: {}, RequestId: {}",
        policyArn, result.getPolicyVersion().getVersionId(),
        result.getPolicyVersion().getIsDefaultVersion(),
        result.getSdkResponseMetadata().getRequestId());
  ...
  protected void deleteNonDefaultPolicyVersions(String policyName) {
    ...
    String policyArn = awsMapper.getPolicyArn(policyName);
    versions.stream().filter(version -> !version.isDefaultVersion())
  ...
  protected void deletePolicyVersion(String policyArn, String versionId) {
    ...
    log.info("Deleted policy version. ARN: {}, VersionId: {}, RequestId: {}", policyArn, versionId,
        result.getSdkResponseMetadata().getRequestId());
  }
