...
public class AwsIamRolePolicyAttachmentClient implements
  ...
  public <BE extends BaseIaasRolePolicyAttachment<?>> IaasRolePolicyAttachment add(BE attachment) {
    ...
    AttachRolePolicyRequest request = new AttachRolePolicyRequest().withPolicyArn(policyArn)
        .withRoleName(attachment.getIaasRole().getId());

    AttachRolePolicyResult result = this.awsClient.attachRolePolicy(request);
    log.info("Attached policy to role. Role: {}, PolicyArn: {}, RequestId: {}",
        attachment.getIaasRole().getId(), policyArn,
  ...
  public boolean delete(IaasRolePolicyAttachmentId id) {
    ...
    try {
      ...
      log.info("Detached policy from role. Role: {}, PolicyArn: {}, RequestId: {}", id.getRoleId(),
          policyArn, result.getSdkResponseMetadata().getRequestId());

      getCache().invalidate(id.getRoleId());
      return true;
    } catch (NoSuchEntityException e) {
      log.error("Failed to delete attachment with role: '{}' and policy: '{}'", id.getRoleId(),
          policyArn);
      return false;
    }
  }
        ...
        public Set<String> load(@Nonnull String roleName) throws Exception {
          ...
          do {
            ListAttachedRolePoliciesRequest request
                = new ListAttachedRolePoliciesRequest().withRoleName(roleName).withMarker(marker);
