...
public class AwsArnMapper {
  ...
  private String getServiceArn(String region, String serviceName, String resourcePrefix,
      String resourceName) {
    String arn = Arn.builder().withAccountId(this.accountId).withPartition(this.partition)
        .withService(serviceName).withRegion(region).withResource(resourcePrefix + resourceName)
        .build().toString();
