...
public class BulkIaasClient<ID, Entity extends Identified<ID>, BaseEntity, UpdateEntity>
  ...
  public BulkIaasClient(BlockingAsyncExecutor executor,
  ...
  public List<Entity> get(Set<ID> ids) {
    List<Supplier<Entity>> suppliers = ids.stream().filter(Objects::nonNull)
        .map(id -> (Supplier<Entity>) () -> iaasClient.get(id)).toList();
  ...
  public <BE extends BaseEntity> List<Entity> add(List<BE> entities) {
    List<Supplier<Entity>> suppliers = entities.stream().filter(Objects::nonNull)
        .map(entity -> (Supplier<Entity>) () -> iaasClient.add(entity)).toList();
  ...
  public <UE extends UpdateEntity> List<Entity> put(List<UE> entities) {
    List<Supplier<Entity>> suppliers = entities.stream().filter(Objects::nonNull)
        .map(entity -> (Supplier<Entity>) () -> iaasClient.put(entity)).toList();
  ...
  public long delete(Set<ID> ids) {
    List<Supplier<Boolean>> suppliers = ids.stream().filter(Objects::nonNull)
        .map(id -> (Supplier<Boolean>) () -> iaasClient.delete(id)).toList();
  ...
  public Map<ID, Boolean> exists(Set<ID> ids) {
    ...
    this.get(ids).stream().filter(Objects::nonNull).map(Identified::getId)
