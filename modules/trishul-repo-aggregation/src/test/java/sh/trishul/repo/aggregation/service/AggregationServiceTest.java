package sh.trishul.repo.aggregation.service;
...
import sh.trishul.repo.aggregation.service.function.AggregationFunction;
...
import sh.trishul.repo.jpa.query.spec.criteria.NullSpec;
...
class AggregationServiceTest {
  ...
  void getAggregations_ReturnsPageOfContentAndTotalCount() {
    ...
    doReturn(List.of(new TestEntity(1L))).when(mRepo).getAggregation(TestEntity.class, selector,
        groupBy, mSpec, PageRequest.of(1, 10, Direction.ASC, "col_1"));
    doReturn(99L).when(mRepo).getResultCount(TestEntity.class,
        new SelectClauseBuilder().select(new NullSpec()), groupBy, mSpec, null);

    Page<TestEntity> page = this.service.getAggregation(TestEntity.class, mSpec,
        AggregationFunction.SUM, new TestPathProvider("col_3"),
        new PathProvider[] {new TestPathProvider("col_1"), new TestPathProvider("col_2")},
        new TreeSet<>(List.of("col_1")), true, 1, 10);

    Page<TestEntity> expected = new PageImpl<>(List.of(new TestEntity(1L)),
        PageRequest.of(1, 10, Direction.ASC, "col_1"), 99L);
  ...
  void getResultCount_ReturnsCountOfRepoCount() {
    ...
    doReturn(100L).when(mRepo).getResultCount(TestEntity.class,
        new SelectClauseBuilder().select(new NullSpec()), groupBy, mSpec, null);
