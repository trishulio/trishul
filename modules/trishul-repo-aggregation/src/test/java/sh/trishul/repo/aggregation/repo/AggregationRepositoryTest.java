...
class AggregationRepositoryTest {
  ...
  void testGetAggregation_ReturnsListOfResultsFromTypedQuery() {
    ...
    doReturn(mTq).when(mResolver).buildQuery(TestEntity.class, Object.class, mSelector, mGroupBy,
        mSpec, PageRequest.of(1, 10));
    doReturn(List.of(new TestEntity(1L))).when(mTq).getResultList();

    List<Object> content = repo.getAggregation(TestEntity.class, Object.class, mSelector, mGroupBy,
        mSpec, PageRequest.of(1, 10));
  ...
  void testGetAggregation_UsesSameClassForReturnClassAndReturnsListOFResultsFromTypedQuery_WhenOverloadedMethodIsUsed() {
    ...
    doReturn(mTq).when(mResolver).buildQuery(TestEntity.class, TestEntity.class, mSelector,
        mGroupBy, mSpec, PageRequest.of(1, 10));
    doReturn(List.of(new TestEntity(1L))).when(mTq).getResultList();

    List<TestEntity> content
        = repo.getAggregation(TestEntity.class, mSelector, mGroupBy, mSpec, PageRequest.of(1, 10));
  ...
  void testGetSingleAggregation_ReturnsSingleResultFromTypedQuery() {
    ...
    TestEntity content = repo.getSingleAggregation(TestEntity.class, TestEntity.class, mSelector,
        mGroupBy, mSpec, PageRequest.of(1, 10));
  ...
  void testResultCount_ReturnsCountOfStream() {
    ...
    Long count
        = repo.getResultCount(TestEntity.class, mSelector, mGroupBy, mSpec, PageRequest.of(1, 10));
