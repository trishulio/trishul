...
public class TenantDataSourceConfigurationProvider
    ...
    public TenantDataSourceConfigurationProvider(
        ...
        this.cache =
                CacheBuilder.newBuilder()
                        .build(
                                new CacheLoader<UUID, DataSourceConfiguration>() {
                                    @Override
                                    public DataSourceConfiguration load(@Nonnull UUID tenantId)
                                            throws Exception {
                                        DataSourceConfiguration config = adminDsConfig;

                                        if (!adminTenant.getId().equals(tenantId)) {
                                            String fqName =
                                                    dsConfigMgr.getFqName(
                                                            globalTenantDsConfig.getSchemaPrefix(),
                                                            tenantId);
                                            config =
                                                    new LazyTenantDataSourceConfiguration(
                                                            fqName,
                                                            globalTenantDsConfig,
                                                            secretsManager);
                                        }
    ...
    public DataSourceConfiguration getConfiguration(UUID tenantId) {
        try {
            ...
            throw new RuntimeException(
                    String.format(
                            "Failed to load datasource configuration for tenantId: '%s' because: %s",
