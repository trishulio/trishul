...
public class JpaJoinerLocalCacheTest {
    ...
    @Test
    public <X, Y>void testJoin_ReturnsCachedJoinFromLocalThread() throws InterruptedException, ExecutionException {
        Key<X, Y> mKey = mock(Key.class);
        Path<X> mPath = mock(Path.class);
        Supplier<Path<X>> mSupplier = mock(Supplier.class);
        doReturn(mPath).when(mSupplier).get();

        CompletableFuture<Path<X>> op1 = CompletableFuture.supplyAsync(() -> this.cache.get(mKey, mSupplier));
        CompletableFuture<Path<X>> op2 = CompletableFuture.supplyAsync(() -> this.cache.get(mKey, mSupplier));

        op1.thenAccept(path -> assertEquals(mPath, path));
        op2.thenAccept(path -> assertEquals(mPath, path));

        CompletableFuture.allOf(op1, op2)
                         .thenRun(() -> verify(mSupplier, times(2)).get())
                         .get();
    }

    @Test
    public <X, Y> void testJoin_ReturnsCachedJoinFromLocalThread_Multithreaded() throws InterruptedException {
        Key<X, Y> mKey = mock(Key.class);
        Path<X> mPath = mock(Path.class);
        Supplier<Path<X>> mSupplier = mock(Supplier.class);
        doReturn(mPath).when(mSupplier).get();
