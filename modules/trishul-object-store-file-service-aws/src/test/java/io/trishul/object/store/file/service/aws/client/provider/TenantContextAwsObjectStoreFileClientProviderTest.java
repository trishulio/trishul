...
public class TenantContextAwsObjectStoreFileClientProviderTest {
  ...
  public void testGetIaasRepository_ReturnsClientFromTenantContext() {
    ...
    doReturn(mS3).when(mFactory).s3Client("REGION", "ACCESS_KEY_ID", "ACCESS_SECRET_KEY",
        "SESSION_TOKEN");

    doReturn(new IaasAuthorization("ACCESS_KEY_ID", "ACCESS_SECRET_KEY", "SESSION_TOKEN",
        LocalDateTime.of(2000, 1, 1, 0, 0))).when(mAuthFetcher).fetch();

    doReturn("OBJECT_STORE").when(mBucketNameProvider).getObjectStoreName();

    IaasRepository<URI, IaasObjectStoreFile, BaseIaasObjectStoreFile<?>, UpdateIaasObjectStoreFile<?>> client
        = provider.getIaasRepository();
