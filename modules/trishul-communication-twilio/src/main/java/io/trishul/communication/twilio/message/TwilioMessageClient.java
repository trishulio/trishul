...
public class TwilioMessageClient
    implements IaasClient<String, TrishulMessage, BaseMessage<?>, UpdateMessage<?>> {
  private static final Logger log = LoggerFactory.getLogger(TwilioMessageClient.class);

  private final String accountSid;
  private final IaasEntityMapper<Message, TrishulMessage> mapper;

  public TwilioMessageClient(String accountSid, IaasEntityMapper<Message, TrishulMessage> mapper) {
    this.accountSid = accountSid;
    this.mapper = mapper;
  }

  @Override
  public TrishulMessage get(String messageSid) {
    try {
      MessageFetcher fetcher = fetcher(accountSid, messageSid);
      Message twilioMessage = fetcher.fetch();
  ...
  @Override
  public <BE extends BaseMessage<?>> TrishulMessage add(BE entity) {
  ...
  @Override
  public <UE extends UpdateMessage<?>> TrishulMessage put(UE entity) {
