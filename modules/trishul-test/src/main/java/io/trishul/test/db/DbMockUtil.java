...
public abstract class DbMockUtil {
    ...
    public static PreparedStatement mockPs(Connection conn, String sql, Object[][] data)
        ...
        doAnswer(inv -> data[cursor.get()][inv.getArgument(0, Integer.class)])
                .when(rs)
                .getObject(anyInt());
        doAnswer(
                        inv -> {
    ...
    public static void createAndSetMockConnection(
            DataSource ds, String username, String schema, String url, boolean autoCommit)
            throws SQLException {
        ...
        doAnswer(
                ...
                .when(ds)
                .getConnection(anyString(), anyString());
    }

    public static Connection mockConnection(
            String username, String schema, String url, boolean autoCommit) throws SQLException {
        Map<String, Object> data = new HashMap<>();
    ...
    public static Connection mockConnection(
        ...
        doAnswer(inv -> data.put("autoCommit", inv.getArgument(0, Boolean.class)))
                .when(conn)
                .setAutoCommit(anyBoolean());
        doAnswer(inv -> data.put("schema", inv.getArgument(0, String.class)))
                .when(conn)
                .setSchema(anyString());
