...
public abstract class DbMockUtil {
  ...
  public static PreparedStatement mockPs(Connection conn, String sql, Object[][] data)
    ...
    doAnswer(inv -> data[cursor.get()][inv.getArgument(0, Integer.class)]).when(rs)
        .getObject(anyInt());
    doAnswer(inv -> {
  ...
  public static Connection mockConnection(Map<String, Object> data, String username, String schema,
    ...
    doAnswer(inv -> data.put("autoCommit", inv.getArgument(0, Boolean.class))).when(conn)
        .setAutoCommit(anyBoolean());
    doAnswer(inv -> data.put("schema", inv.getArgument(0, String.class))).when(conn)
        .setSchema(anyString());
