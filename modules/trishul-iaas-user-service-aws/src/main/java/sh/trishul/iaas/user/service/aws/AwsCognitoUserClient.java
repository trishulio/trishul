package sh.trishul.iaas.user.service.aws;
...
public class AwsCognitoUserClient
  ...
  public IaasUser get(String id) {
    ...
    AdminGetUserRequest req = new AdminGetUserRequest().withUsername(id).withUserPoolId(userPoolId);
    try {
  ...
  public <UE extends UpdateIaasUser<?>> IaasUser update(UE update) {
    ...
    AdminUpdateUserAttributesRequest req = new AdminUpdateUserAttributesRequest()
        .withUserPoolId(userPoolId).withUsername(update.getEmail()).withUserAttributes(attributes);

    AdminUpdateUserAttributesResult result = this.idp.adminUpdateUserAttributes(req);
    log.info("Updated Cognito user attributes. Username: {}, UserPoolId: {}, RequestId: {}",
        update.getEmail(), userPoolId, result.getSdkResponseMetadata().getRequestId());
