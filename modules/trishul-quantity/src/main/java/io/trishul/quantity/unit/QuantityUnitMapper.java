...
public abstract class QuantityUnitMapper {
    ...
    private Map<String, Unit<?>> getAllUnits() {
        try {
            ...
            Function<Field, Object> getValue =
                    field -> {
                        try {
                            ...
                            String msg =
                                    String.format(
                                            "Failed to retrieve the field value because: %s",
                                            e.getMessage());
                            logger.error(msg);
            ...
            Map<String, Unit<?>> unitMap =
                    Arrays.stream(fields)
                            .filter(
                                    field ->
                                            isPublicStaticFinal.test(field.getModifiers())
                                                    && !field.getName().contains("DEFAULT"))
                            .map(getValue)
                            .filter(o -> o instanceof Unit)
                            .collect(
                                    Collectors.toMap(
                                            o -> toSymbol(((Unit<?>) o)), o -> (Unit<?>) o));

            return unitMap;
        } catch (IllegalArgumentException e) {
            String msg =
                    String.format(
                            "Failed to fetch property descriptors for class: %s because %s",
