...
public class TenantIaasVfsServiceTest {
    ...
    public void init() {
        ...
        this.service =
                new TenantIaasVfsService(
                        mResMapper,
                        mPolicyService,
                        mObjectStoreService,
                        mAttachmentService,
                        mIaasBucketCrossOriginConfigService,
                        mIaasPublicAccessBlockService,
                        mBuilder);

        doAnswer(
                        inv ->
                                new IaasRolePolicyAttachment(
                                        inv.getArgument(0, IaasRole.class),
                                    ...
                                    .map(
                                            objectStore ->
                                                    new TenantIaasVfsResources(
                                                            objectStore, policies.next()))
                                    .toList();
                        })
                .when(mResMapper)
                .fromComponents(anyList(), anyList());
    }

    @Test
    public void
            testGet_ReturnsListOfVfsResourcesBuiltFromAllServiceResponse_WhenIdpTenantsAreNotNull() {
        ...
        List<IaasObjectStore> mObjectStores =
                List.of(
                        new IaasObjectStore("OBJECT_STORE_1"),
                        new IaasObjectStore("OBJECT_STORE_2"));
        doReturn(mObjectStores)
                .when(mObjectStoreService)
    ...
    @Test
    public void
            testAdd_ReturnsListOfVfsResourcesBuiltFromAllServiceResponse_WhenIdpTenantsAreNotNull() {
        List<IaasIdpTenant> iaasIdpTenant =
                List.of(new IaasIdpTenant("T1"), new IaasIdpTenant("T2"));
        iaasIdpTenant.get(0).setIaasRole(new IaasRole("IAAS_ROLE_1"));
        iaasIdpTenant.get(1).setIaasRole(new IaasRole("IAAS_ROLE_2"));

        doReturn(new IaasObjectStore("OBJECT_STORE_1"))
                .when(mBuilder)
                .buildObjectStore(iaasIdpTenant.get(0));
        doReturn(new IaasObjectStore("OBJECT_STORE_2"))
                .when(mBuilder)
                .buildObjectStore(iaasIdpTenant.get(1));

        doReturn(new IaasPolicy("VFS_POLICY_1"))
                .when(mBuilder)
                .buildVfsPolicy(iaasIdpTenant.get(0));
        doReturn(new IaasPolicy("VFS_POLICY_2"))
                .when(mBuilder)
                .buildVfsPolicy(iaasIdpTenant.get(1));
        ...
        doAnswer(inv -> inv.getArgument(0, List.class))
                .when(mIaasBucketCrossOriginConfigService)
                .add(anyList());
        doAnswer(inv -> inv.getArgument(0, List.class))
                .when(mIaasPublicAccessBlockService)
                .add(anyList());
    ...
    @Test
    public void
            testPut_ReturnsListOfVfsResourcesBuiltFromAllServiceResponse_WhenIdpTenantsAreNotNull() {
        List<IaasIdpTenant> iaasIdpTenant =
                List.of(new IaasIdpTenant("T1"), new IaasIdpTenant("T2"));
        iaasIdpTenant.get(0).setIaasRole(new IaasRole("IAAS_ROLE_1"));
        iaasIdpTenant.get(1).setIaasRole(new IaasRole("IAAS_ROLE_2"));
    ...
    public void testDelete_DelegatesDelete_WhenIdpTenantsAreNotNull() {
        ...
        doReturn(new IaasRolePolicyAttachmentId("ROLE_1", "POLICY_ID_1"))
                .when(mBuilder)
                .buildVfsAttachmentId("T1");
        doReturn(new IaasRolePolicyAttachmentId("ROLE_2", "POLICY_ID_2"))
                .when(mBuilder)
        ...
        order.verify(mAttachmentService, times(1))
                .delete(
                        Set.of(
                                new IaasRolePolicyAttachmentId("ROLE_1", "POLICY_ID_1"),
        ...
        order.verify(mObjectStoreService, times(1))
                .delete(Set.of("OBJECT_STORE_1", "OBJECT_STORE_2"));
    }
}
